...
export type LoadingBouncingDotsProps = WithoutChildren<WithElementRef<HTMLAttributes<HTMLSpanElement>>> & {
  count?: number;
  label?: string;
};
...
let {
  ...
  label = "読み込み中",
  ...restProps
}: LoadingBouncingDotsProps = $props();
...
<!-- Adapted from Loading UI (bouncing-dots), MIT License. -->
<span
  bind:this={ref}
  {...restProps}
  role="status"
  aria-label={label}
  data-slot="loading-bouncing-dots"
