  ...
  const resolvedOptions: TableOptionsResolved<TData> = mergeObjects(
    {
      state: {},
      onStateChange() {
        // TanStack Table requires a default state-change handler.
      },
    ...
    table.setOptions(() => {
      return mergeObjects(resolvedOptions, options, {
        ...
        onStateChange: (updater: Updater<TableState>) => {
          state = applyTableStateUpdater(state, updater);
...
export function applyTableStateUpdater(state: TableState, updater: Updater<TableState>): TableState {
  if (updater instanceof Function) return updater(state);

  return mergeObjects(state, updater);
}
...
export function mergeObjects<Sources extends readonly MaybeThunk<object>[]>(...sources: Sources): Intersection<{ [K in keyof Sources]: Sources[K] }> {
  ...
  return new Proxy(Object.create(null), {
    get(_, key) {
      ...
      return src === undefined ? undefined : Reflect.get(src, key);
    },
