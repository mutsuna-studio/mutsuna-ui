import { themeDefinitions, defaultThemeKey, contrastColors } from "./tokens.generated.js";
...
export type ThemeTemplateKey = (typeof themeDefinitions)[number]["key"];
...
const lightForeground = contrastColors.lightForeground;
const darkForeground = contrastColors.darkForeground;
const absoluteLightForeground = contrastColors.absoluteLightForeground;
const absoluteDarkForeground = contrastColors.absoluteDarkForeground;
const minimumTextContrastRatio = 4.5;

export const defaultTheme: Theme = createTheme(defaultThemeKey, themeDefinitions.find(({ key }) => key === defaultThemeKey)!.primary);

export const themeTemplates: readonly ThemeTemplate[] = themeDefinitions.map((definition) => ({
  ...createThemeTemplate(definition.key, definition.name, definition.description, definition.primary),
  ...("appearance" in definition ? { appearance: definition.appearance } : {}),
  ...("previewHex" in definition ? { previewHex: definition.previewHex } : {}),
}));
